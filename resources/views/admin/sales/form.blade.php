...
@section('content')
    <form class="space-y-6" method="POST" action="{{ route('admin.sales.store') }}" x-data="{ tab: 'customer', items: [{ product_id: '', quantity: 1, unit_price: 0 }] }">
        ...
        <div class="flex flex-wrap gap-2 border-b border-zinc-200 pb-2 text-sm">
            <button type="button" class="rounded-full px-4 py-2" :class="tab === 'customer' ? 'bg-[#0f2744] text-white' : 'bg-zinc-100 text-zinc-700'" @click="tab = 'customer'">Customer</button>
            <button type="button" class="rounded-full px-4 py-2" :class="tab === 'items' ? 'bg-[#0f2744] text-white' : 'bg-zinc-100 text-zinc-700'" @click="tab = 'items'">Line items</button>
        </div>
